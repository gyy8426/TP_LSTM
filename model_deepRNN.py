...
class Attention(object):
    ...
    def init_params(self, options):
        ...
        BN_gamma_list = []
        '''
        for i in range(options['temp_size']):
            BN_gamma_list.append(norm_weight_single(options['ctx_dim']))
        params['BN_gamma'] = np.array(BN_gamma_list)
        params['BN_beta'] = np.zeros(shape=[options['temp_size'],options['ctx_dim']],dtype='float32')
        '''
        params = self.layers.get_layer('ff')[0](params, nin=options['ctx_dim'], nout=options['mu_dim'],
                                                prefix='ff_init_tslstm_hid')
        params = self.layers.get_layer('ff')[0](params, nin=options['ctx_dim'], nout=options['mu_dim'],
                                                prefix='ff_init_tslstm_mem')
        params = self.layers.get_layer('lstm')[0](params, nin=options['ctx_dim'],
                                                  dim=options['mu_dim'], prefix='ts_lstm')        

        params = self.layers.get_layer('ff')[0](params, nin=options['mu_dim'], nout=options['mu_dim'],
                                                prefix='ff_ts2mu')
        # decoder: LSTM
        params = self.layers.get_layer('lstm')[0](params, nin=options['dim_word'],
                                                  dim=options['tu_dim'], prefix='tu_lstm')
        mu_lstm_dimctx = options['mu_dim'] + options['ctx_dim']
        params = self.layers.get_layer('lstm_cond')[0](options, params, nin=options['tu_dim'],
                                                       dim=options['mu_dim'], dimctx=mu_lstm_dimctx,
                                                       prefix='mu_lstm')
    ...
    def build_model(self, tparams, options):
        ...
        if options['init_ts']:
            init_ts_lstm_hid = self.layers.get_layer('ff')[1](tparams, ctx_mean, activ='tanh',
                                                   prefix='ff_init_tslstm_hid')
            init_ts_lstm_mem = self.layers.get_layer('ff')[1](tparams, ctx_mean, activ='tanh',
                                                   prefix='ff_init_tslstm_mem')                                               
        ...
        ts_lstm = self.layers.get_layer('lstm')[1](tparams, ctx_temp_pooling_T,
                                                   init_state = init_ts_lstm_hid,
                                                   init_memory = init_ts_lstm_mem,
                                                   prefix='ts_lstm')
        if options['get_tslstm'] == 'mean':
            hid_ts_lstm = ts_lstm[0].dimshuffle(1,0,2)
            hid_ts_lstm = T.mean(hid_ts_lstm, axis=1, keepdims=False)
        if options['get_tslstm'] == 'final':
            hid_ts_lstm = ts_lstm[0][-1]
        hid_ts_lstm = self.layers.get_layer('ff')[1](tparams, hid_ts_lstm, activ='tanh',
                                               prefix='ff_ts2mu')
        if options['use_dropout']:
            hid_ts_lstm_drop = self.layers.dropout_layer(hid_ts_lstm, use_noise, trng)
        # decoder
        mu_lstm_input = T.concatenate([ctx_mean, hid_ts_lstm_drop],axis= -1)
        ...
        mu_lstm = self.layers.get_layer('lstm_cond')[1](options, tparams, tu_lstm[0],
                                                        mask=mask, context=mu_lstm_input,
                                                        one_step=False,
                                                        trng=trng,
                                                        use_noise=use_noise,
                                                        prefix='mu_lstm')
        ...
        for eidx in xrange(max_epochs):
            ...
            for idx in self.engine.kf_train:
                ...
                if validFreq != -1 and np.mod(uidx, validFreq) == 0:
                    ...
                    if len(history_errs) > 1 and valid_B4 >= np.array(history_errs)[:-1,11].max() and valid_meteor >= 0.33:
